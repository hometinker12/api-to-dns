...
- Restoring a backup onto a new Docker host with a different `ENCRYPTION_KEY` no longer leaves Fernet settings unreadable. Settings, DNS zone configs, and the ACME account key are re-encrypted to this installation's key instead of requiring the archive key to replace Compose `.env` values. An unreadable `ssl_enabled` row fails closed at bootstrap; set `SSL_ENABLED=0` to start HTTP for recovery.
...
- Application version metadata aligned to **0.8.7** (`VERSION`, OpenAPI, `pyproject.toml`, Docker label, Compose pin).
