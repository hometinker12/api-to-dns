...
- Operational Python logging and system identity helpers live in `src/operational_logging.py` and `src/system_identity.py`. SMTP alerting lives in `src/alerting.py`; remote syslog get/set/apply lives in `src/remote_syslog.py`. `src/activity_logging.py` re-exports them for compatibility.
