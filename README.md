...
>
> The image runs as non-root uid/gid **10001**. Compose pins `hometinker12/api-to-dns:0.8.6`, enables `read_only`, drops capabilities, and binds published ports to **localhost** (`127.0.0.1:8001` and `127.0.0.1:8443`). If you upgrade from a root-owned named volume, fix ownership once (do not start the app as root):
>
> ```bash
