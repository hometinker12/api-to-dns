...
>
> The image runs as non-root uid/gid **10001**. Compose pins `hometinker12/api-to-dns:0.8.8`, enables `read_only`, drops capabilities, and binds published ports to **localhost** (`127.0.0.1:8001` and `127.0.0.1:8443`). If you upgrade from a root-owned named volume, fix ownership once (do not start the app as root):
>
> ```bash
...
- Export and restore configuration under **Settings → Backup** (global admin only): password encryption is required for settings, users, DNS zones, API key hashes, SSL files, and application secrets. Alert rules and audit logs may be exported without a password. Restore remains backward-compatible with legacy plaintext archives, is destructive for selected categories, and shows an inline progress dialog. Restoring settings, zones, or SSL files uses the archive `ENCRYPTION_KEY` to decrypt Fernet data, then re-encrypts it with this installation's key so a new Docker host with a different `.env` can boot. Restoring Application secrets alone still writes durable secrets and restarts the app.
