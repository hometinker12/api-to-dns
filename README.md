...
- Open the **DNS browser** from a zone name on the Dashboard (or the DNS Zones list). The page loads blank with a search bar and type filter (All records, A, AAAA, CNAME, TXT, MX, NS, SRV, CAA, PTR, SOA). Enter an exact relative name such as `@` or `www` to look up records. Viewing requires `dns_zones.read`; add/edit/delete require `dns_zones.update`. Edits replace the whole RRset for that name and type. SOA is view-only; apex NS cannot be modified. PTR mutations are limited to reverse zones (`in-addr.arpa` / `ip6.arpa`). Canonical value formats: MX `priority exchange`, SRV `priority weight port target`, CAA `flags tag value`. Credentials stay server-side (session auth, not API keys).
